...
func main() {
	...
	var deltaParamsID = int16(landingID - paramsID)
	...
	// Params ID
	checkErr(binary.Write(buf, binary.LittleEndian, deltaParamsID))
	fmt.Println(base64.RawURLEncoding.EncodeToString(buf.Bytes()), "landingID+deltaParamsID")
	// Hash bot name
	checkErr(binary.Write(buf, binary.LittleEndian, adler32.Checksum([]byte(botName))))
	fmt.Println(base64.RawURLEncoding.EncodeToString(buf.Bytes()), "landingID+deltaParamsID+Checksum")
	payload := buf.Bytes()
	...
	fmt.Println(finalURL, "landingID+deltaParamsID+Checksum+aes")
